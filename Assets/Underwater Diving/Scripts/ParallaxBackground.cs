...
public class ParallaxBackground : MonoBehaviour {
    ...
    void Start()
    {
        ...
        // Arka planın başlangıç konumlarını doğru ayarlamak için
    ...
    {
        // Bu fonksiyon, tüm katmanların başlangıç konumlarını ayarlamak için kullanılır.
        float startPosX = cameraTransform.position.x;
        for (int i = 0; i < layers.Length; i++)
        {
            float offset = i * backgroundSize;
            layers[i].position = new Vector3(startPosX + offset, layers[i].position.y, layers[i].position.z);
        }
    }
}
